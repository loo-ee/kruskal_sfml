...
int main(int argc, char* argv[]) {
    sf::RenderWindow window(sf::VideoMode(1280, 720), "Kruskal Algorithm Visualizer");
    sf::Font font1;
    ...
    std::unordered_map<Vertex*, Vertex* >* parentSubGraphs = manager.getParentSubGraphs();
    std::unordered_map<Vertex*, int>* subGraphsRanks = manager.getSubGraphsRanks();
    ...
    while (window.isOpen()) {
        ...
        if (isCalculating) {
            if (isFirstRun) {
                ...
                subGraphsRanks->clear();

                manager.makeSets();
                manager.sortEdges();

                currentEdgeIter = edges->begin();
            }

            if (currentEdgeIter == edges->end()) {
                system("clear");
                ...
                Edge* currentEdge = (*currentEdgeIter);

                std::string vertex0Name = currentEdge->src;
                std::string vertex1Name = currentEdge->dst;

                Vertex* vertex0 = (*vertices)[vertex0Name];
                Vertex* vertex1 = (*vertices)[vertex1Name];

                Vertex* vertex0Root = manager.findParent(vertex0);
                Vertex* vertex1Root = manager.findParent(vertex1);

                if (vertex0Root != vertex1Root) {
                    MST_Path.push_back(currentEdge);
                    manager.unionSet(vertex0Root, vertex1Root);
                }
        ...
        for (auto it = vertices->begin(); it != vertices->end(); it++) {
            ...
            for (std::string& currentNeighbor : currentVertex->neighbors) {
                ...
                std::vector<Edge* >::iterator foundEdge = std::find_if(MST_Path.begin(), MST_Path.end(), 
                    [currentEdge](const Edge* resultEdge) {
...
void addVerticesToManager(GraphManager& manager) {
    ...
    for (std::unordered_map<std::string, std::vector<std::string>>::iterator it = 
        verticesMap.begin(); it != verticesMap.end(); it++) {
